...
class UnseenSegment:
    def __init__(self, model_path, device, confidence=0.7):
        ...
        cfg.MODEL.ROI_HEADS.NUM_CLASSES = 1 #设置感兴趣区域（ROI）头部的类别数为 1，表明模型用于单类别分割任务。
        cfg.MODEL.SEM_SEG_HEAD.NUM_CLASSES = 1 #设置语义分割头部的类别数为 1。
        cfg.MODEL.ROI_BOX_HEAD.CLS_AGNOSTIC_BBOX_REG = True #设置感兴趣区域（ROI）框头部的类别无关边界框回归
        cfg.MODEL.ROI_MASK_HEAD.CLS_AGNOSTIC_MASK = True #设置感兴趣区域（ROI）掩膜头部的类别无关掩膜
        cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = confidence #设置模型在测试时的置信度阈值
        cfg.MODEL.DEVICE = device #设置模型的设备，可以是 "cpu" 或 "cuda"。
